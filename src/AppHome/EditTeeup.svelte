        ...
        if (id) {
            ...
            fetch('https://teeupapp-21db3.firebaseio.com/teeups.json', {
                ...
                .then(res => {
                    ...
                    return res.json() //from this data retuned from firebase we want to extract unique firebase id.
                })
                .then(data => {
                    //is what the the res.json returned
                    teeups.addTeeup({
                        ...teeupData,
                        isFavourite: false,
                        isGoing: false,
                        id: data.name, //data.name is the unique firebase id.
                    })
                })
                .catch(err => {
                    console.log(err)
